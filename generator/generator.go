...
func (g *OpenAPIv3Generator) addSchemasForMessagesToDocumentV3(d *v3.Document, messages []*protogen.Message) {
	...
	for _, message := range messages {
		...
		var required []string
		g.extractFields(message, definitionProperties, &required)
...
func (g *OpenAPIv3Generator) extractFields(message *protogen.Message, definitionProperties *v3.Properties, required *[]string) {
	...
	for _, field := range message.Fields {
		...
		if _, ok := ExtractFlatten(message, field); ok {
			g.extractFields(field.Message, definitionProperties, required)
			continue
		}
